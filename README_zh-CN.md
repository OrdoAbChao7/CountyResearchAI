...
> 一个 LLM 辅助的县域产业研究原型。输入县名和可选方向后，它会收集已配置的公开材料、保留来源链接，并生成供人工审阅的 Markdown **研究草稿**。三种模式分别覆盖现状快照、近现代兴衰时间线和县域长周期轨迹。
...
- **候选方向发现** — 未提供方向时，系统会让已配置的 LLM 基于检索材料排序候选项。选出的方向只是起始假设，使用草稿前应人工复核。
...
- **LLM 辅助综合** — 提示词模板将草稿组织为各模式对应的章节；每个章节仍需要来源核对和人工修订。
...
- **提供方接口分离** — LLM、搜索、存储和报告实现相互隔离，使提供方改动的代码影响范围可控；兼容性仍需配置与回归验证。
- **Mock 路径** — 未配置 API Key 时，项目可用合成输入演练控制流。Mock 输出仅用于本地开发和测试。
